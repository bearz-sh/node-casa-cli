import { expandTemplate } from "@app/tasks/hbs";
import { cat, copyFile, exists, IFileInfo, IOError, mkdir, readDotEnv, walkDir, writeText } from "@app/io/fs";
import { execaSync as exec } from "execa";
import { dirname, join } from "node:path";
import { CASA_DATA_DIR, IS_WINDOWS } from "@app/os/index";
import * as dockerCompose from '@app/tasks/docker/compose';
import * as jp from '@app/util/json-path';
import * as envConfig from '@app/tasks/env';
...
export function up(app: string, env?: string, expand = false, force = false) {
    ...
    dockerCompose.up({
        ...
        envFile: join(appDir, 'compile.env'),
        wait: true,
        noRecreate: true
    });
}
